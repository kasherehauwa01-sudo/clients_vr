...
function SearchBar({ value, onOpen }: { value: string; onOpen: () => void }) {
  return <button className="search-paper" type="button" aria-haspopup="dialog" onClick={onOpen}>
    ...
    <span className={value ? 'search-value' : 'search-placeholder'}>{value || 'Поиск и фильтры'}</span><span className="filter-hint">Настроить</span>
  </button>;
}
type MultiFilterProps = { title: string; values: string[]; selected: string[]; onChange: (values: string[]) => void };
function MultiFilter({ title, values, selected, onChange }: MultiFilterProps) {
  const toggle = (value: string) => onChange(selected.includes(value) ? selected.filter(item => item !== value) : [...selected, value]);
  return <details className="filter-group"><summary><span>{title}</span>{selected.length > 0 && <span className="filter-count">{selected.length}</span>}</summary><div className="filter-values">{values.length ? values.map(value => <label key={value}><input type="checkbox" checked={selected.includes(value)} onChange={() => toggle(value)} />{value}</label>) : <p>Нет доступных значений</p>}</div></details>;
}
