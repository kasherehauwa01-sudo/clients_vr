  ...
  return <main>
    ...
    <header className="topbar">
      <nav className="tabs" aria-label="Основная навигация">
        ...
        <button className={activeTab === 'registry' ? 'selected' : ''} onClick={() => setActiveTab('registry')}>Контрагенты</button>
        <button className={activeTab === 'settings' ? 'selected' : ''} onClick={() => setActiveTab('settings')}>Настройки</button>
      </nav>
    </header>
    {activeTab === 'settings' ? <Settings active={helpTab} onChange={setHelpTab} onUpload={() => fileInputRef.current?.click()} notice={notice} /> : <>
      <section className="search-paper">
        ...
        <input className="search" aria-label="Поиск контрагентов" placeholder="Поиск по клиентам, email, телефонам, фирме..." value={q} onChange={e => { setQ(e.target.value); setPage(1); }} />
      </section>
