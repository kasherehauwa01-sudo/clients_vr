...
function EmailReportDialog({ managers, onClose }: { managers: string[]; onClose: () => void }) {
  ...
  const uploadExclusions = async (file?: File) => {
    ...
    try {
      ...
      setExclusionEmails(result.emails); setMessage(`Загружено адресов: ${result.count}. Предыдущий список заменён.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); }
