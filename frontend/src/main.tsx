...
function EmailReportDialog({ managers, onClose }: { managers: string[]; onClose: () => void }) {
  ...
  return <SimpleDialog title="Отчет «Обновление email»" onClose={onClose}>
    <nav className="report-tabs" aria-label="Разделы настройки отчета" role="tablist"><button type="button" role="tab" aria-selected={dialogTab === 'filters'} className={dialogTab === 'filters' ? 'selected' : ''} onClick={() => { setDialogTab('filters'); setExclusionCategory(null); }}>Фильтры</button><button type="button" role="tab" aria-selected={dialogTab === 'exclusions'} className={dialogTab === 'exclusions' ? 'selected' : ''} onClick={() => setDialogTab('exclusions')}>Исключения</button></nav>
...
function ClientCard({ c }: { c: any }) { const phones = c.phones || []; const cardEmail = c.raw_email_source_known ? c.raw_email : c.emails?.join(', '); return <div className="card"><h2>{c.name}</h2><Block title="Информация о записи" rows={[["Дата загрузки", c.first_import_at ? formatMoscowTime(c.first_import_at) : null], ["Дата обновления", c.updated_at ? formatMoscowTime(c.updated_at) : null]]} /><Block title="Все поля XLS" rows={[["Наименование", c.name], ["Тип цены", c.price_type], ["Менеджер", c.manager], ["Дата рождения", c.birth_date], ["Email", cardEmail], ["Телефоны прочие", c.raw_common_phones || phones.filter((p: any) => p.type === 'common').map((p: any) => p.phone).join(', ')], ["Места торговли", c.trade_places?.join(', ')], ["Телефоны для СМС и рассылки", c.raw_sms_phones || phones.filter((p: any) => p.type === 'sms').map((p: any) => p.phone).join(', ')], ["Руководитель", c.director], ["Фирма", c.company], ["Контактное лицо", c.contact_person], ["Источник клиента", c.client_source], ["Дата последней покупки", c.last_purchase_date], ["Вид покупателя", c.buyer_type], ["Вид контрагента", c.counterparty_type]]} /></div>; }
