  ...
  return <main>
    <header className="topbar">
      ...
      {activeTab === 'registry' && <div className="header-actions">
        ...
        <button className="primary-action" onClick={() => fileInputRef.current?.click()}>⬆️ Импорт Excel</button>
      </div>}
    </header>
    ...
    {activeTab === 'help' ? <Help active={helpTab} onChange={setHelpTab} /> : <>
      <section className="toolbar"><input placeholder="Поиск по клиентам, email, телефонам, фирме..." value={q} onChange={e => { setQ(e.target.value); setPage(1); }} /><a className="button tonal" href={`${API_BASE_URL}/clients-export.xlsx`}>⬇️ Экспорт</a></section>
