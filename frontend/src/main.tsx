...
function ClientCard({ c }: { c: any }) { return <div className="card"><h2>{c.name}</h2><Block title="Основная информация" rows={[["Фирма", c.company], ["Тип цены", c.price_type], ["Менеджер", c.manager], ["Статус", STATUS_LABELS[c.status as ClientStatus] || c.status]]} /><Block title="Контакты" rows={[["Контактное лицо", c.contact_person], ["Руководитель", c.director], ["Email", c.emails?.join(', ')], ["Все телефоны", c.phones?.map((p: any) => `${p.phone} (${p.type})`).join(', ')]]} /><Block title="Дополнительно" rows={[["Дата рождения", c.birth_date], ["Места торговли", c.trade_places?.join(', ')], ["Источник клиента", c.client_source], ["Дата последней покупки", c.last_purchase_date], ["Вид покупателя", c.buyer_type], ["Вид контрагента", c.counterparty_type]]} /><Block title="История" rows={[["Первый импорт", c.first_import_at?.slice(0, 10)], ["Последнее обновление", c.updated_at?.slice(0, 10)], ["Последний импорт", c.last_import_at?.slice(0, 10)], ["Файл", c.last_import_file]]} /></div>; }
