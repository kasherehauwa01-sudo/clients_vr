...
function App() {
  ...
  return <main>
    ...
    {activeTab === 'help' ? <Help active={helpTab} onChange={setHelpTab} /> : <>
      <section className="toolbar"><input placeholder="Мгновенный поиск по клиентам, email, телефонам..." value={q} onChange={e => { setQ(e.target.value); setPage(1); }} /><select value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })}><option value="">Все статусы</option><option value="active">Активный</option><option value="archived">Архивный</option><option value="out_of_stock">Нет в наличии</option></select><select value={filters.has_email} onChange={e => setFilters({ ...filters, has_email: e.target.value })}><option value="">Email: все</option><option value="true">Есть Email</option><option value="false">Нет Email</option></select><select value={filters.sort} onChange={e => setFilters({ ...filters, sort: e.target.value })}><option value="name">Сортировка: наименование</option><option value="company">Фирма</option><option value="manager">Менеджер</option><option value="birth_date">Дата рождения</option><option value="updated_at">Обновление</option><option value="last_import">Импорт</option></select><a className="button" href={`${API_BASE_URL}/clients-export.xlsx`}>⬇️ Экспорт</a></section>
